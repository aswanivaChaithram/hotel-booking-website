...
const Navbar = () => {
    ...
    const { openSignIn } = useClerk(); // from Clerk authentication library
    const { user } = useUser();        // gets the currently signed-in user
    const navigate = useNavigate();    // to navigate between pages (React Router)
    ...
    return (
            <nav className={`fixed top-0 left-0 w-full flex items-center 
                ...
                <div className="hidden md:flex items-center gap-4 lg:gap-8">
                    {navLinks.map((link, i) => (
                        <a key={i} href={link.path} offset="100" className={`group flex flex-col gap-0.5 
                ...
                <div className="flex items-center gap-4 md:hidden cursor-pointer">
                    ...
                    <img src={assets.menuIcon} alt="" onClick={() => {setIsMenuOpen(true)}} 
                ...
                <div className={`fixed top-0 left-0 w-full h-screen bg-white 
                    text-base flex flex-col md:hidden items-center justify-center
                     gap-6 font-medium text-gray-800 transition-all duration-500 
                     ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    <button className="absolute top-6 right-6 cursor-pointer" onClick={() => setIsMenuOpen(false)}>
                        <img src={assets.closeIcon} alt="" className="h-4.5" />
                    </button>
                    ...
                    { !user &&
                    (<button onClick={openSignIn} className="bg-black text-white px-8 py-2.5 
                    rounded-full ml-1 transition-all duration-500 cursor-pointer">
                        Login
                    </button>)
                     }
